...
export function validateGasConstraints(params: {
  gasLimit: bigint | undefined;
  constraints: Action["constraints"];
  venueName: string;
}): void {
  const { gasLimit, constraints, venueName } = params;
  if (constraints?.maxGas === undefined) return;

  if (gasLimit === undefined) {
    throw new Error(`${venueName} could not estimate gas while max_gas is enabled`);
  }
  if (gasLimit > constraints.maxGas) {
    throw new Error(
      `${venueName} gas estimate ${gasLimit.toString()} exceeds max_gas ${constraints.maxGas.toString()}`
    );
  }
}
